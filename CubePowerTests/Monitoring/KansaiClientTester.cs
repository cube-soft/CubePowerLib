/* ------------------------------------------------------------------------- */
///
/// Monitoring/KansaiClientTester.cs
///
...
namespace CubePowerTests.Monitoring
{
    ...
    /// 
    /// <summary>
    /// KansaiClient クラスをテストするためのクラスです。
    /// </summary>
    ///
    /* --------------------------------------------------------------------- */
    [TestFixture]
    public class KansaiClientTester
    {
        /* ----------------------------------------------------------------- */
        ///
        /// TestFromFile
        ...
        public void TestFromFile()
        {
            try
            {
                var client = new CubePower.Monitoring.KansaiClient();
                Assert.AreEqual(CubePower.Monitoring.Area.Kansai, client.Area);

                var filename = "kansai.csv";
                ...
                Assert.AreEqual(1416, response.Usage);
                Assert.AreEqual(2500, response.Capacity);
                Assert.AreEqual(57, response.UsageRatio);
                ...
                Assert.AreEqual(0, response.Time.Second);
                Assert.AreEqual("万kW", response.Unit);
                Assert.AreEqual(1857, response.Usage);
                Assert.AreEqual(2500, response.Capacity);
                Assert.AreEqual(74, response.UsageRatio);
