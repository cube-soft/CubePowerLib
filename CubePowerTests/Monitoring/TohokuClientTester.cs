/* ------------------------------------------------------------------------- */
///
/// Monitoring/TohokuClientTester.cs
///
...
namespace CubePowerTests.Monitoring
{
    ...
    /// 
    /// <summary>
    /// TohokuClient クラスをテストするためのクラスです。
    /// </summary>
    ///
    /* --------------------------------------------------------------------- */
    [TestFixture]
    public class TohokuClientTester
    {
        /* ----------------------------------------------------------------- */
        ///
        /// TestFromFile
        ...
        public void TestFromFile()
        {
            try
            {
                var client = new CubePower.Monitoring.TohokuClient();
                Assert.AreEqual(CubePower.Monitoring.Area.Tohoku, client.Area);

                var filename = "tohoku.csv";
                ...
                Assert.AreEqual(1138, response.Capacity);
                Assert.AreEqual(764, response.Usage);
                Assert.AreEqual(67, response.UsageRatio);
                ...
                Assert.AreEqual("万kW", response.Unit);
                Assert.AreEqual(1138, response.Capacity);
                Assert.AreEqual(945, response.Usage);
