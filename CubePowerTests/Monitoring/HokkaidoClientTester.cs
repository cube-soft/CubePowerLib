/* ------------------------------------------------------------------------- */
///
/// Monitoring/HokkaidoClientTester.cs
///
...
namespace CubePowerTests.Monitoring
{
    /* --------------------------------------------------------------------- */
    ///
    /// HokkaidoClientTester
    /// 
    /// <summary>
    /// HokkaidoClient クラスをテストするためのクラスです。
    /// </summary>
    ///
    /* --------------------------------------------------------------------- */
    [TestFixture]
    public class HokkaidoClientTester
    {
        /* ----------------------------------------------------------------- */
        ///
        /// TestFromFile
        ...
        public void TestFromFile()
        {
            try
            {
                ...
                Assert.AreEqual(487, response.Capacity);
                Assert.AreEqual(310, response.Usage);
                ...
                Assert.AreEqual("万kW", response.Unit);
                Assert.AreEqual(487, response.Capacity);
                Assert.AreEqual(346, response.Usage);
                Assert.AreEqual(71, response.UsageRatio);
