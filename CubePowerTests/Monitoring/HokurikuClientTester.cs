/* ------------------------------------------------------------------------- */
///
/// Monitoring/HokurikuClientTester.cs
///
...
namespace CubePowerTests.Monitoring
{
    /* --------------------------------------------------------------------- */
    ///
    /// HokurikuClientTester
    /// 
    /// <summary>
    /// HokurikuClient クラスをテストするためのクラスです。
    /// </summary>
    ///
    /* --------------------------------------------------------------------- */
    [TestFixture]
    public class HokurikuClientTester
    {
        /* ----------------------------------------------------------------- */
        ///
        /// TestFromFile
        ...
        public void TestFromFile()
        {
            try
            {
                var client = new CubePower.Monitoring.HokurikuClient();
                Assert.AreEqual(CubePower.Monitoring.Area.Hokuriku, client.Area);

                var filename = "hokuriku.csv";
                ...
                Assert.AreEqual(411, response.Capacity);
                Assert.AreEqual(280, response.Usage);
                Assert.AreEqual(68, response.UsageRatio);
                ...
                Assert.AreEqual("万kW", response.Unit);
                Assert.AreEqual(411, response.Capacity);
                Assert.AreEqual(356, response.Usage);
                Assert.AreEqual(87, response.UsageRatio);
