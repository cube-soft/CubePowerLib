...
namespace CubePower.Monitoring
{
    ...
    public class HokkaidoClient : Client
    {
        ...
        {
            if (time >= DateTime.Today) return "http://denkiyoho.hepco.co.jp/data/juyo_hokkaidou.csv";

            _today = false;
            var format = "http://denkiyoho.hepco.co.jp/data/{0}_juyo_hokkaidou.csv";
            if (time >= new DateTime(2010, 1, 1)) return String.Format(format, time.Year);
